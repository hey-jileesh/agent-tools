---
name: prd
description: "Interactive requirement analyst that turns a fuzzy idea, feature request, or Jira epic/story into a precise, business-level Product Requirement Document (PRD) using Rich Hickey's 'Design in Practice' — Socratic questioning, decision matrices, and phased design (describe, diagnose the real problem, delimit, direction, design). Stays at the business problem/solution level for business, systems, and domain analysts: objectives, use cases, business rules, and data/entities/conceptual data models when relevant — not code, architecture, APIs, or physical schema. Input can be a Jira issue key (read via the jira-issue-reader skill) or context the user provides. Triggers: 'write a PRD', 'create a product requirements doc', 'spec this feature', 'turn this idea or JIRA-123 into requirements', 'what problem are we actually solving', 'requirement analysis', 'scope this work'. Use whenever someone wants to define, scope, or write up product/feature requirements."
